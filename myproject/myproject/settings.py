...
DOWNLOAD_TIMEOUT = 3  # 单请求超时时间（秒），根据网络状况调整

# 并发控制（重要优化点）
CONCURRENT_REQUESTS = 32  # 全局最大并发数（建议根据机器性能设置）
CONCURRENT_REQUESTS_PER_DOMAIN = 16  # 单域名并发上限（建议与DOWNLOAD_DELAY配合）
CONCURRENT_REQUESTS_PER_IP = 0  # 设为0禁用，避免与PER_DOMAIN冲突（除非有特殊IP限制）

# 请求频率控制
DOWNLOAD_DELAY = 0  # 基础延迟（秒），建议2-5秒
RANDOMIZE_DOWNLOAD_DELAY = True  # 启用±50%随机延迟（2±1秒）
AUTOTHROTTLE_ENABLED = True  # 启用动态限速（建议保留）
AUTOTHROTTLE_START_DELAY = 0  # 新增：初始延迟（建议大于DOWNLOAD_DELAY）
AUTOTHROTTLE_MAX_DELAY = 0  # 新增：最大延迟上限
